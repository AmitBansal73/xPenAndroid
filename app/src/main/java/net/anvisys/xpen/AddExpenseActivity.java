...
public class AddExpenseActivity extends AppCompatActivity
    ...
    public void SaveExpense(View v) {
        ...
        try {
            if (editAmount.getText().toString().matches("") || editRemarks.getText().toString().matches("")) {
                Snackbar.make(v, "Fill Required Fields", Snackbar.LENGTH_LONG).setAction("Action", null).show();
                ...
                */
                {
                    ...
                    try {
                        String reqBody = "{\"ActivityID\":\"" + ei.ActivityId + "\",\"ItemName\":\"" + ei.expense_item + "\",\"ExpenseAmount\":\"" + ei.expense + "\",\"ReceiveAmount\":" + 0 + ",\"ExpenseDescription\":\"" + ei.Remarks + "\",\"ExpenseDate\":\"" + ei.ExpenseDate + "\",\"SelectedRow\":\"" + 0 + "\",\"Status\":\"" + "Added" + "\"}";
                        ...
                        JsonObjectRequest jsArrayRequest = new JsonObjectRequest(Request.Method.POST, url, jsonObject, new Response.Listener<JSONObject>() {
                            ...
                            public void onResponse(JSONObject jObj) {
                                try {
                                    ...
                                    if (Response.matches("OK")) {
                                        Snackbar.make(getCurrentFocus(), "Updated Successfully", Snackbar.LENGTH_LONG).setAction("Action", null).show();
                                        AddExpenseActivity.this.finish();
                                    } else {
                                        Snackbar.make(getCurrentFocus(), "Failed to Update", Snackbar.LENGTH_LONG).setAction("Action", null).show();
                                    }
                            ...
                            public void onErrorResponse(VolleyError error) {
                                if (error.networkResponse == null) {
                                    Snackbar.make(getCurrentFocus(), "Network Error, Try Again", Snackbar.LENGTH_LONG).setAction("Action", null).show();
                                }
                                progressBar.setVisibility(View.GONE);
                                btnSave.setEnabled(true);
    ...
    // private void SaveExpenseAtServer()
